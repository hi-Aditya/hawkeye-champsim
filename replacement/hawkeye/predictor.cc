...
HawkeyePredictor::HawkeyePredictor(std::size_t num_entries, int counter_bits) : num_entries_(num_entries > 0 ? num_entries : 1), max_value_((1 << (counter_bits > 0 ? counter_bits : 1)) - 1), init_value_(1 << ((counter_bits > 0 ? counter_bits : 1) - 1)), counters_(num_entries_, init_value_){}

std::size_t HawkeyePredictor::index_of(uint64_t pc) const{
  const uint64_t hashed_pc = pc ^ (pc >> 12);
  if(isPowerOfTwo(num_entries_)){
